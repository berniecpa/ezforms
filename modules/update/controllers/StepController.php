        ...
        if (Yii::$app->request->post('language')) {
            ...
            try {
                ...
                if ($response->isOk) {
                    ...
                    $license = !empty($response->data['license']) ? $response->data['license'] : '';
                    if ($status == 1) {
                        Yii::$app->session->set('purchaseCode', $purchaseCode);
                        Yii::$app->session->set('license', $license);
                        if (!empty($message)) {
                ...
                Yii::error($e);
                Yii::$app->session->setFlash('warning', Yii::t('app', 'This server does not meet the minimum requirements for updating the software.'));
                return $this->redirect(['step/1']);
            }
