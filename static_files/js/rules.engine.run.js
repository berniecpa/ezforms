...
$(document).ready(function () {
    ...
    if (!String.prototype.format) {
        String.prototype.format = function () {
            ...
            if (arguments.length) {
                ...
                str = str.replace(new RegExp("\{\{([a-zA-Z0-9_]+)\}\}", "gi"), "");
    ...
    var setTargetElement = function (data, originalElement) {
        ...
        // Replace default value, if original value exists
        if (data.find("original") === "value") {
            value = data.find("original", "originalValue");
        }
        ...
        if (!!operator){
            if ($.isArray(operands)) {
                $.each(operands, function (index, operand) {
                    ...
                    if (component.length > 1) { // Used by checkbox components
                    ...
                    } else if (component.length === 1) { // All components
    ...
    var rules = function () {
        ...
        var actionsAdapter = {
            toShow: function (data) {
                if (data.find("target") === "field") {
                    var field = findField(data.find("target", "targetField"));
                    if (!field.is(':visible')) {
                        field.trigger('ef.show');
                    }
                    ...
                    field.parent().closest('div.form-group').show();
                    field.trigger('ef.shown')
                } else if (data.find("target") === "element") {
                    var elm = findElement(data.find("target", "targetElement"));
                    if (!elm.is(':visible')) {
                        elm.trigger('ef.show');
                    }
                    elm.show().trigger('ef.shown');
                }
            },
            toHide: function (data) {
                if (data.find("target") === "field") {
                    var field = findField(data.find("target", "targetField"));
                    if (field.is(':visible')) {
                        field.trigger('ef.hide');
                    }
                    field.hide();
                    field.parent().closest('div.form-group').hide();
                    field.trigger('ef.hidden');
                } else if (data.find("target") === "element") {
                    var elm = findElement(data.find("target", "targetElement"));
                    if (elm.is(':visible')) {
                        elm.trigger('ef.hide');
                    }
                    elm.hide().trigger('ef.hidden');
                }
            },
            ...
            formatNumber: function (data) {
                if (options.twoLetterLanguageCode) {
                    numeral.locale(options.twoLetterLanguageCode);
                }
                if (data.find("target") === "field") {
                    var field = findField(data.find("target", "targetField")); // field
                    if (field) {
                        var fieldValue = numeral(parseFloat(field.val())).format(data.find("format"));
                        field.val(fieldValue);
                    }
                } else if (data.find("target") === "element") {
                    var element = findElement(data.find("target", "targetElement"));
                    if (element) {
                        var elementValue = numeral(parseFloat(element.text())).format(data.find("format"));
    ...
    formEl.find(":input:not(select)").on('input click', function () {
        rules();
    });

    formEl.find("select").on('change', function () {
        rules();
    });
